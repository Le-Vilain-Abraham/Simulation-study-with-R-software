...
performance_measures <- function(t.star, lambda, rho, beta, dataset, gamma, PH){
  
  #True difference in RMSTs
  if (PH){
    true.delta <- true_rmst_difference_PH(t.star, 0.0001,lambda, rho, beta, gamma)
  } else {
    true.delta <- true_rmst_difference_NPH(t.star, 0.0001, lambda, rho, beta, gamma, 90)
  }
